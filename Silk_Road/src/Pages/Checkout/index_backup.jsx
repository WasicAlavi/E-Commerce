...
const Checkout = () => {
  ...
  const buyNowOrder = location.state?.buyNowOrder;
  const isBuyNowOrder = location.state?.fromProduct;

  // Use buy now order data or mock cart data
  const cartItems = buyNowOrder ? [
    {
      id: buyNowOrder.product.id,
      name: buyNowOrder.product.name,
      price: buyNowOrder.product.price,
      quantity: buyNowOrder.quantity,
      image: buyNowOrder.product.images?.[0] || '/product1.jpg',
      selectedSize: buyNowOrder.selectedSize,
      selectedColor: buyNowOrder.selectedColor
    }
  ] : [
    {
      id: 1,
  ...
  // Calculate totals based on buy now order or cart items
  const subtotal = buyNowOrder ? buyNowOrder.subtotal : cartItems.reduce((total, item) => total + (item.price * item.quantity), 0);
  const shipping = buyNowOrder ? buyNowOrder.shipping : (subtotal >= 3000 ? 0 : 200);
  const total = buyNowOrder ? buyNowOrder.total : subtotal + shipping;
    ...
    try {
      ...
      if (buyNowOrder) {
        ...
        orderData = {
          ...
          items: [{
            product_id: buyNowOrder.product.id,
            quantity: buyNowOrder.quantity,
            price: buyNowOrder.discountedPrice,
            originalPrice: buyNowOrder.originalPrice,
            discount: buyNowOrder.product.discount || 0,
            name: buyNowOrder.product.name,
            image: buyNowOrder.product.images?.[0] || '/product1.jpg',
            size: buyNowOrder.selectedSize,
            color: buyNowOrder.selectedColor,
          }],
          subtotal: buyNowOrder.subtotal,
          discount: buyNowOrder.originalPrice * buyNowOrder.quantity - buyNowOrder.subtotal,
          shipping: buyNowOrder.shipping,
          total: buyNowOrder.total,
          coupon: null,
        };
      } else {
        // Cart order (fallback)
    ...
    <div className="bg-[#EDF6E5] min-h-screen py-8 font-montserrat">
      <div className="container mx-auto px-4">
        ...
        <div className="flex items-center gap-2 mb-6">
          <Link to={isBuyNowOrder ? "/" : "/cart"} className="text-[#40513B] hover:text-[#9DC08B]">
            <FaArrowLeft />
          </Link>
          <h1 className="text-3xl font-bold text-[#40513B]">
            {isBuyNowOrder ? 'Buy Now Checkout' : 'Checkout'}
          </h1>
        </div>
              ...
              <div className="space-y-4 mb-6">
                {buyNowOrder ? (
                  <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                    <img 
                      src={buyNowOrder.product.images?.[0] || '/product1.jpg'} 
                      alt={buyNowOrder.product.name}
                      className="w-16 h-16 object-cover rounded"
                    />
                    <div className="flex-1">
                      <h3 className="font-medium text-[#40513B]">{buyNowOrder.product.name}</h3>
                      <p className="text-sm text-gray-600">
                        Qty: {buyNowOrder.quantity}
                        {buyNowOrder.selectedSize && ` | Size: ${buyNowOrder.selectedSize}`}
                        {buyNowOrder.selectedColor && ` | Color: ${buyNowOrder.selectedColor}`}
                      </p>
                      <p className="text-sm font-medium text-[#9DC08B]">
                        ৳{buyNowOrder.discountedPrice} × {buyNowOrder.quantity}
                      </p>
                    </div>
                  </div>
                ) : (
